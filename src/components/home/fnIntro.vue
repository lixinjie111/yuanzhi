<template>
  <div>
    <div class="intro_contant" ref="introContant" @mouseleave="hideFnintro">
        <div class="point_container" v-for="(item,index) in introData" :key="index" @click="goDetails(index)" ref="point_container">
  ...
  export default {
      ...
      methods:{
          ...
          goDetails(index){
              ...
              if(typeSymbol == 'yuyan' && index == 0){
                  this.$router.push({
                      path:'/aiSmartAppDetail'
                  });
              }
             if(typeSymbol == 'yuyin' && index == 0){
                  this.$router.push({
