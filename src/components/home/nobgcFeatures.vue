  ...
  <div>
      ...
      <div class="features_container" v-else>
          ...
          <div class="features_point" ref="point">
              <div class="features_point_container" v-for="(item,index) in featurData.featur" :key="index" @click="goFeatureDetail(item)">
                  <img :src="item.img" alt="">
                  <div>{{item.Text}}</div>
                  <div>{{item.desc}}</div>
              </div>
  ...
  data () {
    return {
        ...
        beforeFourIntroData0:[
            {
                title:'词法分析',
                desc:'提供分词、词性标注、命名实体识别三大功能，支撑自然语言的准确理解'
            },
            {
                title:'句子成分分析',
                desc:'帮助机器实现对文本内容的准确理解，实现语义理解、语义匹配及实体数据库搭建等应用'
            },
            {
                title:'句法依存分析',
                desc:'输入中文句子，即可获得该句的依存句法结构信息'
            },
            {
                title:'文本分类',
                desc:'对文章按照内容类型进行自动分类，支持娱乐、体育、科技等26个主流内容类型，为文章聚类、文本内容分析等应用提供基础技术支持'
            },
            {
                title:'情感分析',
                desc:'对包含主观信息的文本进行情感倾向性判断，可支持在线训练模型调优效果，为口碑分析、话题监控、舆情分析等应用提供帮助'
            },
        ],
        ...
        beforeFourIntroData3:[
            {
                title:'视频内容分析',
                desc:'通过多维AI技术，对视频进行智能分析，输出视频内容的泛标签从而提高搜索准确度和用户推荐视频的曝光量'
            },
            {
                title:'视频内容审核',
        ...
        if(this.type != 0){
            if(this.type == 1){
                ...
                for(var i = 0;i< obserDom.length;i++){
                    obserDom[i].style='border: 1px solid #EBECF0;border-radius:3px;padding:45px 34px;';
                }
            }
            else{
